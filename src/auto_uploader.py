...
from google_photos_uploader.uploader import (
    upload_photos as core_upload_photos,
)  # noqa: E402
from google_photos_uploader.uploader import _collect_media_files  # noqa: E402
from google_photos_uploader.utils import find_sd_card  # noqa: E402
...
def batch_create_media_items(upload_tokens, album_name=None, verbose=False):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"バッチ作成成功: {len(success_data.get('success', []))}個, 失敗: {len(success_data.get('failed', []))}個"
            )
            return success_data
...
def upload_photos(
    ...
    if verbose:
        logger.debug(
            f"{len(photo_files)} 件のメディアファイルを検出: {photo_files[:10]}"
        )  # 先頭10件のみ表示
    if not photo_files:
        ...
        logger.info(
            "アップロードする写真はありませんが、SDカードの写真をスライドショーで表示します"
        )
        # photo_files から最大100枚をピックアップ
        limited_files = photo_files[: min(len(photo_files), 100)]
        logger.info(
            f"スライドショー用に {len(limited_files)}/{len(photo_files)} 枚を使用します"
        )
    ...
    # 3. 写真のアップロード処理
    success = core_upload_photos(
        Path(dcim_path), album_name=album_name, verbose=verbose
    )
    return success


def show_uploaded_slideshow(
    fullscreen=True,
    recent=True,
    current_only=False,
    interval=5,
    ...
    try:
        ...
        if sys.platform == "win32":
            subprocess.Popen(
                command, creationflags=subprocess.CREATE_NEW_CONSOLE, env=env
            )
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="SDカードから自動的に写真をアップロードする"
    )
    parser.add_argument(
        "--album", type=str, default=DEFAULT_ALBUM, help="アップロード先のアルバム名"
    )
    parser.add_argument(
        "--slideshow",
        action="store_true",
        help="アップロードと同時にスライドショーを表示する",
    )
    parser.add_argument(
        "--no-fullscreen",
        action="store_true",
        help="スライドショーをフルスクリーンで表示しない",
    )
    parser.add_argument(
        "--fullscreen",
        action="store_true",
        help="スライドショーをフルスクリーンで表示する",
    )
    parser.add_argument(
        "--all-photos",
        action="store_true",
        help="すべての写真をスライドショーに表示する（デフォルトは最新のみ）",
    )
    parser.add_argument(
        "--current-only", action="store_true", help="最新の写真のみ表示する"
    )
    # スライドショー用の追加オプション
    parser.add_argument(
        "--slideshow-interval",
        type=int,
        default=5,
        help="スライドショーの画像表示間隔（秒）",
    )
    parser.add_argument(
        "--random", action="store_true", help="スライドショーをランダム順で表示する"
    )
    parser.add_argument(
        "--no-pending",
        action="store_true",
        help="アップロード予定/失敗ファイルを含めない",
    )
    parser.add_argument("--verbose", action="store_true", help="詳細なログを出力する")
    parser.add_argument(
        "--bgm",
        nargs="*",
        help="BGMとして再生する音楽ファイルまたはディレクトリ（複数指定可）",
    )
    parser.add_argument(
        "--random-bgm", action="store_true", help="BGMをランダムに再生する"
    )
    ...
    if not sd_path or not (sd_path / DCIM_PATH).exists():
        logger.error(
            "SDカードが見つかりません。SDカードを挿入してから再度実行してください。"
        )
        return
