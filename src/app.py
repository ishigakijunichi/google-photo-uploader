...
def setup_directories():
    ...
    try:
        ...
        if not CREDENTIALS_FILE.exists():
            logging.warning(
                "credentials.jsonが見つかりません。Google認証の設定が必要です。"
            )
            logging.warning(
                "Google Cloud Consoleから認証情報をダウンロードし、~/.google_photos_uploader/credentials.jsonに配置してください。"
            )
    ...
    try:
        ...
        for proc in psutil.process_iter(["pid", "cmdline"]):
            ...
            if any(p in cmdline for p in patterns):
                try:
                    logger.info(
                        f"スライドショープロセス停止: PID={proc.pid} CMD='{cmdline}' (signal={sig})"
                    )
    ...
    try:
        ...
        for proc in psutil.process_iter(["pid", "cmdline"]):
            ...
            if any(p in cmdline for p in patterns):
                try:
                    logger.info(
                        f"アップローダープロセス停止: PID={proc.pid} CMD='{cmdline}' (signal={sig})"
                    )
                    os.kill(proc.pid, sig)
                except ProcessLookupError:
                    # 既に終了している
...
def cleanup():
    ...
    try:
        ...
        subprocess.run(
            ["pkill", "-f", "python.*(slideshow|album_slideshow).py"], check=False
        )
...
def start_upload():
    try:
        ...
        if uploader_running:
            return (
                jsonify(
                    {"status": "error", "message": "アップロードは既に実行中です。"}
                ),
                400,
            )
        ...
        try:
            ...
            from auto_uploader import DCIM_PATH  # 定数のみ利用
            ...
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": "内部エラー: SDカード検出モジュールの読み込みに失敗しました",
                    }
                ),
                500,
            )
...
def stop_upload():
    try:
        ...
        if is_process_running("auto_uploader.py"):
            logger.warning(
                "SIGTERM で auto_uploader が終了しなかったため SIGKILL を送信します"
            )
            kill_auto_uploader_processes(force=True)

        if is_process_running("slideshow.py") or is_process_running(
            "album_slideshow.py"
        ):
            logger.warning(
                "SIGTERM でスライドショーが終了しなかったため SIGKILL を送信します"
            )
...
def unmount_sd():
    try:
        ...
        if is_process_running("slideshow.py") or is_process_running(
            ...
            logger.warning(
                "SIGTERM でスライドショーが終了しなかったため SIGKILL を送信"
            )
            kill_slideshow_processes(force=True)

        if is_process_running("auto_uploader.py"):
            logger.warning(
                "SIGTERM で auto_uploader が終了しなかったため SIGKILL を送信"
            )
        ...
        if platform.system() == "Darwin":  # macOS
            ...
            if target_volumes:
                for vol in target_volumes:
                    ...
                    message = (
                        "DCIM フォルダを含むボリュームのアンマウントに失敗しました"
                    )
            else:
            ...
            message = (
                "このプラットフォームではSDカードのアンマウントをサポートしていません"
            )
        else:
            message = (
                "このプラットフォームではSDカードのアンマウントをサポートしていません"
        ...
        return (
            jsonify({"message": f"アンマウント中にエラーが発生しました: {str(e)}"}),
            500,
        )
    except Exception as e:
...
def get_console_log():
    ...
    try:
        ...
        return jsonify(
            {"logs": [f"コンソールログの取得中にエラーが発生しました: {str(e)}"]}
        )
...
def check_status():
    ...
    try:
        uploader_running = is_process_running("auto_uploader.py")
        slideshow_running = is_process_running("slideshow.py") or is_process_running(
            "album_slideshow.py"
        )
...
def get_albums():
    ...
    try:
        ...
        return (
            jsonify(
                {"error": f"アルバムリスト取得中にエラーが発生しました: {e.stderr}"}
            ),
            500,
        )
    except Exception as e:
...
def start_slideshow():
    try:
        ...
        if uploader_running:
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": "アップロードが実行中です。先に停止してください。",
                    }
                ),
                400,
            )
        ...
        if slideshow_running:
            return (
                jsonify(
                    {"status": "error", "message": "スライドショーは既に実行中です。"}
                ),
                400,
            )
        ...
        return (
            jsonify({"status": "error", "message": f"エラーが発生しました: {e}"}),
            500,
        )
